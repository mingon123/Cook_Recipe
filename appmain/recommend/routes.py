...
def show_recommend():
    ...
    return render_template('recommend.html', recommended_recipes=recommended_recipes)
...
#유사한 레시피 가져옴
@recommend.route('/api/recommend/similar/<int:articleNo>', methods=['GET'])
...
#레시피 상세포인트 표시 엔드포인트
@recommend.route('/display_article/<int:articleNo>', methods=['GET'])
...
#로그인 기록 가져옴(사이드바)
@recommend.route('/api/article/recent_user_visits', methods=['GET'])
...
#로그아웃
@recommend.route('/api/logout', methods=['POST'])
def logout():
    session.clear()
